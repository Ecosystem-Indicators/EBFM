...
##' @title Skip line function. this automaticallu get the number of rows based on the ewe version
...
## if you updated R you will want to run this code just to avoid conflicts with old packages
...
mainDir <- "/home/por07g/Documents/Projects/Lenfest/Indicators_tool"
InputsubDir <- "EBS output files/ecosim_Climate control with fishing"
OutsubDirStep1 <- "Lenfest_output"
OutsubDirStep2 <- "EBS-ecosim_Climate_control_with_fishing"
BosubDir <- "EBS output files/ecosim_2021 baseline"
```
...
if(!dir.exists(file.path(mainDir, OutsubDirStep1))) dir.create(file.path(mainDir, OutsubDirStep1))
if(!dir.exists(file.path(mainDir, OutsubDirStep2))) dir.create(file.path(mainDir, OutsubDirStep2))
...
discardsBySpecies <- df3 %>%
    ...
    dplyr::summarise(TotalDiscards = sum(discard_tonnes, na.rm = TRUE))
...
discard_stats <- df3 %>% dplyr::group_by(FleetName, Year) %>%
...
if (load_effort_data > 0) {
    ...
    if (catch_in_effort_file < 1){
        dfFleetDat <- merge(dfEffort, dfCatchFleet, by=c("Year","FleetName"))
    } else {
        dfFleetDat <- dfEffort
    }
    ...
    for (iLoop in 1:nLoop) {
        ...
        check_name <- FleetNames[counter1:counter2]
        extractC <- dplyr::filter(dfFleetDat, FleetName %in% check_name)
...
origQ <- read.csv("consumption_matrix.csv", header = T, row.names=1, check.names=FALSE)
...
for (iYear in minYear:maxYear) {
    ...
    nDiet <- reshape2::melt(as.matrix(dietData))
    nDiet <- nDiet[nDiet$value != 0, ]
    Supgraph <- graph_from_data_frame(d = nDiet)
    set_vertex_attr(Supgraph, "label", value = l$vertexes[,2])
    edges <- get.edgelist(Supgraph)   # Can also be extracted using E(g)
    food_tidy <- as_tbl_graph(Supgraph, directed = TRUE)
    centRes   <- centrality(food_tidy)
    d_in      <- degree(Supgraph, mode = "in", normalized = FALSE)
    d_out     <- degree(Supgraph, mode = "out", normalized = FALSE)
